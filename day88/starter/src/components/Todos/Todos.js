...
const Todos = () => {
  ...
  const formatTodoText = useCallback((text, index) => {          /**Memoize a function useCallback*/
    return `${text.toLowerCase()} (${index + 1} of 500)`;
  },[]);

  return (
    <div className={styles.container}>
    <Suspense fallback={<p>Loading...</p>}>   {/**Spliting components into chunck using Suspence */}
      <Confetti
        size={size}
