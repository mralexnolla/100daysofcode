...
//import getIconOptions from './getIconOptions';    instead of importing from here 
...
const Profile = () => {
  ...
  useEffect(() => {
     const load = async () => {
      const getIconOptions = await import('./getIconOptions')  /**Split component, i used asyn await to split the component */
  ...
  const isUsernameValid = useMemo(() => (checkUsernameValidity(name)), [name]);  /** Memoize a value */
